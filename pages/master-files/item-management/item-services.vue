...
export default {
  ...
  methods: {
    ...
    async initialize() {
      ...
      params = params + `&wareHouseItems=true`;
    ...
    async saveToLocation(){
      let res = await this.apiItemServicesToLocation(this.selected_item.id)
      if(res.status == 200){
    ...
    async submit(code) {
      ...
      if (this.islocation) return this.saveToLocation();
      if (this.isdelete) return this.remove();
      let res
      if (this.isedit)res = await this.apiUpdateItemandServices(this.payload.id, this.payload);
  ...
  computed: {
    checkPermission() {
      ...
      if (!this.can("delete_ItemMaster")) hideActions.push("delete-btn");
      if (!this.can("add_ItemMaster")) hideActions.push("add-btn");
      if (!this.can("edit_ItemMaster")) hideActions.push("edit-btn");
