<template>
  <v-card elevation="1" v-if="can('browse_purchaseRequestMaster')">
...
<script>
import DataFilter from "~/forms/procurements/filter_forms/PurchaseRequest.vue";
import DataForm from "~/forms/procurements/PurchaseRequest.vue";
import PurchaseHelper from "~/mixins/PurchaseHelper.vue";
import AppHeader from "~/includes/procurements/purchase_request/AppHeader.vue";
import { mapGetters } from "vuex";
import PurchaseRequest from "~/mixins/API/procurement/PurchaseRequest.js"
export default {
  mixins: [PurchaseHelper,PurchaseRequest],
  components: {
    DataFilter,
    DataForm,
    AppHeader,
  ...
  methods: {
    ...
    async submit(code) {
      if (this.$auth.user.details.passcode != code || code == null) {
        this.notification.messages = [];
      ...
      let res = {};
      if (this.isedit)
        res = await this.apiUpdatePurchaseRequest(this.payload.id, fd);
      else res = await this.apiCreatePurchaseRequest(fd);
    ...
    async initialize() {
      ...
      let res = await this.apiGetAllPurchaseRequest(params);
    ...
    editItem(item) {
      ...
      setTimeout(() => {
        this.showForm = true;
        if (this.$auth.user.details) {
          this.payload.requested_by = this.$auth.user.details.name;
          this.payload.department = this.$auth.user.details.warehouse.warehouse_description;
        }
      }, 50);
    },
    addItem() {
    ...
    async approvePR() {
      let res = await this.apiApprovePurchaseRequestItems(this.payload);
      if (res.status == 200) {
    ...
    async remove() {
      let res = await this.apiRemovePurchaseRequest(this.payload.id);
    ...
    viewRecord(item) {
      ...
      Object.assign(this.payload, item);
      this.isaction = true;
      if (this.payload.pr_RequestedBy == this.$auth.user.details.id) {
        this.isedit = true;
      }
  ...
  computed: {
    ...
    checkSideBtn() {
      ...
      if (!this.isedit && this.$auth.user.id != this.payload.user_id) {
        return ["edit", "delete"];
      }
    },
    checkPermission() {
