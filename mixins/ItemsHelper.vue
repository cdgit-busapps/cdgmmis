...
export default {
  methods: {
    ...
    hasTab(tab) {
      if (this.$auth.user.details.role.name == "administrator") {
        if (tab == "consultant") {
          return false;
        }
      } else if (this.$auth.user.details.role.name == "consultant") {
    ...
    hasFilter(field) {
      if (
        this.$auth.user.details.role.name == "staff" ||
        this.$auth.user.details.role.name == "department head"
      ) {
    ...
    hasItem(item){
       return item.ware_house_items.some(item => item.warehouse_Id == this.$auth.user.details.branch_id)
    }
  },
};
</script>