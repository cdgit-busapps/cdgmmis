      ...
      <template v-slot:default>
        ...
        <tbody>
          <tr v-for="(canvas, index) in canvases" :key="index">
            ...
            <td class="text-center">
              <v-icon :disabled="$auth.user.details.role.name!='purchaser'" color="error" @click="$emit('delete', canvas)">mdi-delete</v-icon>
            </td>
          </tr>
        </tbody>
      </template
...
export default {
  ...
  methods:{
    setIsRecommended(canvas){
      if(this.$auth.user.details.role.name!='purchaser') return
