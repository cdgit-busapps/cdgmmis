      ...
      <template v-slot:default>
        <thead>
          <tr>
            ...
            <th class="text-left" v-if="$auth.user.details.role.name != 'administrator' && $auth.user.details.role.name != 'consultant'">Qty</th>
            <th class="text-left" v-if="$auth.user.details.role.name != 'administrator' && $auth.user.details.role.name != 'consultant'">UOM</th>
            <th class="text-left" v-if="isapprove">APPD Qty</th>
            <th class="text-left" v-if="isapprove">APPD UOM</th>
            <th class="text-left">Action</th>
          </tr>
        </thead>
        <tbody v-if="!isremoving">
          <tr v-for="(item, index) in items" :key="index">
            ...
            <td v-if="$auth.user.details.role.name != 'administrator' && $auth.user.details.role.name != 'consultant'">
              <v-text-field
            ...
            <td v-if="isapprove">
              <v-text-field
                ...
                solo
                dense
                :readonly="$auth.user.details.role.name == 'administrator' || $auth.user.details.role.name == 'consultant'"
                hide-details="auto"
                type="number"
              ></v-text-field>
            </td>
...
import PurchaseRequest from "~/mixins/API/procurement/PurchaseRequest"
export default {
  mixins:[PurchaseRequest],
  props: {
    items: {
      type: Array,
      default: () => [],
  ...
  methods: {
    ...
    async removeItem(index) {
      ...
      if (this.isedit) {
        if (this.items[index].item_Id) {
          let res = await this.apiRemovePurchaseRequestItem(this.items[index].id);
          if (res.status == 200) {
    ...
    async onFileChange(file, item) {
      ...
      if (item.item_Id && this.isedit) {
        ...
        let res = await this.apiUpdatePurchaseRequestItemAttachment(item.id, fd);
      }
      setTimeout(() => {
        this.isup = false
      }, 20);
