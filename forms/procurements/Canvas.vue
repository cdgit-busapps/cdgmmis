...
export default {
  ...
  methods: {
    ...
    async approveCanvas(remarks=null){
      ...
      let res = await this.apiApproveCanvas({items: payload})
    ...
    async submitCanvas(){
      ...
      let res = await this.apiSubmitCanvas({items: payload})
      if(res.status == 200){
    ...
    async fetchPR() {
      ...
      let res = await this.apiGetPurchaseRequest(this.pr_id);
